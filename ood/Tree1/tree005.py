...
class BST:
    ...
    def printTree2(self, node, level = 0,start = "Root"):
        if node != None:
            print('       ' * level,end="")
            print(f"{start}----{node.data}")
            self.printTree2(node.left, level + 1,'L')
            self.printTree2(node.right, level + 1,'R')
    ...
    def dupwayfix(self , node,way):
        if (node == None):
            return
        self.insertfix(node.data,way)
        self.dupwayfix(node.left)
        self.dupwayfix(node.right)

    def flipbst(self , node,lvflip,lvnow = 0):
        if (node == None):
            return
        if lvflip <= lvnow+1:
            temp = node.left
            ...
            node.right = temp
        self.flipbst(node.left,lvflip,lvnow+1)
        self.flipbst(node.right,lvflip,lvnow+1)
...
print(" *** Mirror Tree ***")
T = BST()
str1 = input('Enter nodes in level-order,depth : ')
val,lv = str1.split(',')
inp = [int(i) for i in val.split(' ')]
lv = int(lv)
print(f"before mirror: {inp}")
for i in inp:
    T.insertcomplete(i)
T.printTree2(T.root)
T.flipbst(T.root,lv)
